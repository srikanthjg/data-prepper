package org.opensearch.dataprepper.plugins.sink.sagemaker;
 ...
 */
@DataPrepperPlugin(name = "sagemaker", pluginType = Sink.class, pluginConfigurationType = SagemakerSinkConfig.class)
public class S3Sink extends AbstractSink<Record<Event>> {
    ...
    private final SagemakerSinkConfig sagemakerSinkConfig;
    private volatile boolean sinkInitialized;
    private final SagemakerSinkService sagemakerSinkService;
    ...
    public S3Sink(final PluginSetting pluginSetting,
                  final SagemakaerSinkConfig sagemkaerSinkConfig,
                  final PluginFactory pluginFactory,
                  final SinkContext sinkContext,
                  final AwsCredentialsSupplier awsCredentialsSupplier,
                  final ExpressionEvaluator expressionEvaluator) {
        super(pluginSetting);
        this.sagemakerSinkConfig = sagemkaerSinkConfig;
        this.sinkContext = sinkContext;
        final PluginModel codecConfiguration = sagemkaerSinkConfig.getCodec();
...
//        final S3GroupManager s3GroupManager = new S3GroupManager(s3SinkConfig, s3GroupIdentifierFactory, bufferFactory, codecFactory, s3Client, bucketOwnerProvider);
        ...
        s3SinkService = new S3SinkService(sagemkaerSinkConfig, sagemkaerOutputCodecContext, RETRY_FLUSH_BACKOFF, pluginMetrics);
    }
    ...
    public void doOutput(final Collection<Record<Event>> records) {
        sagemakerSinkService.output(records);
    }
}
