...
public class S3SinkService {
     ...
     */
    public SagemakerSinkService(final SagemakerSinkConfig sagemkaerSinkConfig,
                         final OutputCodecContext codecContext,
                         final Duration retrySleepTime,
                         final PluginMetrics pluginMetrics) {
        this.sagemakerSinkConfig = sagemakerSinkConfig;
        this.codecContext = codecContext;
        this.retrySleepTime = retrySleepTime;
        reentrantLock = new ReentrantLock();

        maxEvents = sagemakerSinkConfig.getThresholdOptions().getEventCount();
        maxBytes = sagemakerSinkConfig.getThresholdOptions().getMaximumSize();
        maxCollectionDuration = sagemakerSinkConfig.getThresholdOptions().getEventCollectTimeOut();

        bucket = sagemakerSinkConfig.getBucketName();
        maxRetries = sagemakerSinkConfig.getMaxUploadRetries();
    ...
    void output(Collection<Record<Event>> records) {
        ...
        try {
            for (Record<Event> record : records) {
                ...
                try {
                    //write to sagemaker s3 endpoint
